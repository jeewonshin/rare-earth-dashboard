...
try:
    with open("data/papers.json", "r", encoding="utf-8") as f:
        ...
        existing_urls = set(p["url"] for p in old_data.get("items", []))
    print(f"\n기존 논문 수: {len(existing_urls)}건")
except:
    print("\n기존 데이터 없음 (첫 실행)")

today = datetime.now().date()
for p in papers:
    is_new_url = p["url"] not in existing_urls
    try:
        pub_date    = datetime.strptime(p["date"][:10], "%Y-%m-%d").date()
        is_new_date = (today - pub_date).days <= 7
    except:
        is_new_date = False
    p["is_new"] = is_new_url or is_new_date

new_count = sum(1 for p in papers if p["is_new"])
print(f"신규 논문: {new_count}건 / 전체: {len(papers)}건")
