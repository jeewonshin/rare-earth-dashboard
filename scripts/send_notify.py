...
# ── 워드클라우드 이미지 경로 탐색 ────────────────────────────────────────
wc_ko_path = find_image_path('wc_news_ko.png')
wc_en_path = find_image_path('wc_news_en.png')
print(f'  워드클라우드 KO: {"발견 " + wc_ko_path if wc_ko_path else "없음"}')
print(f'  워드클라우드 EN: {"발견 " + wc_en_path if wc_en_path else "없음"}')
...
if is_wednesday:
    ...
    if wc_ko_path:
        html += '<img src="cid:wc_ko" style="width:100%;border-radius:8px;border:1px solid #e2e8f0" />'
    else:
        html += '<p style="color:#aaa;font-size:12px;padding:20px;text-align:center;border:1px solid #e2e8f0;border-radius:8px">이미지 없음</p>'
    html += '</div>'
    html += '<div style="flex:1;min-width:280px">'
    html += '<p style="color:#666;font-size:12px;margin:0 0 4px 0">&#x1F30D; 해외 뉴스 키워드</p>'
    if wc_en_path:
        html += '<img src="cid:wc_en" style="width:100%;border-radius:8px;border:1px solid #e2e8f0" />'
    else:
        html += '<p style="color:#aaa;font-size:12px;padding:20px;text-align:center;border:1px solid #e2e8f0;border-radius:8px">이미지 없음</p>'
    html += '</div>'
    html += '</div>'
...
if wc_ko_path:
    ...
    img_ko.add_header('Content-ID', '<wc_ko>')
    img_ko.add_header('Content-Disposition', 'inline', filename='wc_news_ko.png')
    msg_related.attach(img_ko)
    print('  KO 이미지 CID 첨부 완료')

if wc_en_path:
    with open(wc_en_path, 'rb') as f:
        img_en = MIMEImage(f.read(), _subtype='png')
    img_en.add_header('Content-ID', '<wc_en>')
    img_en.add_header('Content-Disposition', 'inline', filename='wc_news_en.png')
...
try:
    with smtplib.SMTP('smtp.gmail.com', 587) as server:
        ...
        server.sendmail(GMAIL_USER, all_recipients, msg_outer.as_string())
    print('메일 발송 완료!')
